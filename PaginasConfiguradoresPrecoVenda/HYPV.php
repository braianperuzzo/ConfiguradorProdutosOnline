...
function calcular_preco(PDO $pdo, array $rowPessoa, array $post, string $companyCode, string $categoria, string $codigoEstado)
{
    ...
    $colunaBase = "HY{$categoria}BR";
    $colunaEixoEntrada = "HY{$categoria}EE";
    $colunaAcessorioFixacao = "HY{$categoria}{$hyaf}";
    $colunaAcessoriosSaida = "HY{$categoria}{$hyas}";
    $colunaEspecial = "HY{$categoria}ESP";
    $colunaPintura = null;
    if (strpos($hyopc, 'PINTCZ') !== false) {
        $colunaPintura = "HY{$categoria}PINTCZ";
    } elseif (strpos($hyopc, 'ACOR') !== false) {
        $colunaPintura = "HY{$categoria}ACOR";
    }
    $colunaBlindado = "{$categoria}BLS";

    $lookupRedutor = ['HYLN', 'HYBR'];
    $valRedutor = [$hyln, $hybr];
    $precoRedutor = procvm($pdo, '_USR_CONF_HYPV', $lookupRedutor, $colunaBase, $valRedutor);
    $precoEixoEntrada = (strpos($hycm, 'EE') !== false)
        ? procvm($pdo, '_USR_CONF_HYPV', $lookupRedutor, $colunaEixoEntrada, $valRedutor)
        : 0;
    $precoAcessorioFixacao = ($hyaf !== 'N')
        ? procvm($pdo, '_USR_CONF_HYPV', $lookupRedutor, $colunaAcessorioFixacao, $valRedutor)
        : 0;
    $precoAcessoriosSaida = ($hyas !== 'N')
        ? procvm($pdo, '_USR_CONF_HYPV', $lookupRedutor, $colunaAcessoriosSaida, $valRedutor)
        : 0;
    $precoEspecial = (in_array($hybr, ['104','114','1003','1103'], true) && $hycm === '90' && $hycp === 'B14')
        ? procvm($pdo, '_USR_CONF_HYPV', $lookupRedutor, $colunaEspecial, $valRedutor)
        : 0;
    $precoPintura = $colunaPintura
        ? procvm($pdo, '_USR_CONF_HYPV', $lookupRedutor, $colunaPintura, $valRedutor)
        : 0;
    $precoBlindado = (strpos($hyopc, 'BLS') !== false)
        ? procvm($pdo, '_USR_CONF_HYPVBLS', ['HYLN','HYBR','HYPM'], $colunaBlindado, [$hyln,$hybr,$hypm])
        : 0;

    if ((strpos($hycm, 'EE') !== false && !$precoEixoEntrada) ||
        ($hyln === '1.P' && $hyaf === 'BT' && $precoAcessorioFixacao != 0) ||
        (($hyln !== '1.P' && $hybr !== '211A') && $hyaf !== 'N' && !$precoAcessorioFixacao) ||
        ($hyas !== 'N' && !in_array($hybr, ['862C','863C','1002','1003','1102','1103'], true) && !$precoAcessoriosSaida) ||
        (in_array($hybr, ['104','114','1003','1103'], true) && $hycm === '90' && $hycp === 'B14' && !$precoEspecial) ||
        ((strpos($hyopc, 'PINTCZ') !== false || strpos($hyopc, 'ACOR') !== false) && !$precoPintura) ||
        (strpos($hyopc, 'BLS') !== false && !$precoBlindado)) {
        $precoVendaRedutor = 0;
    } else {
        $precoVendaRedutor = $precoRedutor + $precoEixoEntrada + $precoAcessorioFixacao +
            $precoAcessoriosSaida + $precoEspecial + $precoPintura + $precoBlindado;
    }

    $tabelaPrecoMotor = $hymo === 'S' ? "_USR_CONF_MOPV_{$estado}" : '';
    $tabelaOpcMotor = $hymo === 'S' ? "_USR_CONF_MOOPCPV_{$estado}" : '';

    $colunaMotor = $hymo === 'S' ? "MO{$categoria}" . ($post['MOCP'] ?? '') : '';
    $colunaOpcMotor = null;
    if ($hymo === 'S') {
    ...
    $precoPinturaMotor = ($hymo === 'S' && $colunaPinturaMotor)
        ? procvm(
            $pdo,
            $tabelaPrecoMotor,
    ...
    $precoVendaVariador = ($hyva === 'S')
        ? procvm(
            $pdo,
            '_USR_CONF_VAPV',
            ['VALN', 'VAPT', 'VACM', 'VASCP'],
            "VA{$categoria}",
            [$post['VALN'] ?? '', $post['VAPT'] ?? '', $hycm, $post['VASCP'] ?? '']
        )
        : 0;

    $colunaAdaptacao = $hypl === 'S' ? "PL{$categoria}" : '';
    $diametro = $hypl === 'S'
        ? (($post['PLBU'] ?? 'N') === 'N' ? ($post['PLDE'] ?? '') : ($post['PLBU'] ?? ''))
        : '';
    $precoVendaAdaptacao = ($hypl === 'S')
        ? procvm($pdo, '_USR_CONF_PLASPV', ['PLCM', 'PLDE'], $colunaAdaptacao, [$hycm, $diametro])
        : 0;

    if ($precoVendaRedutor == 0 ||
        ($hymo === 'S' && $precoVendaMotor == 0) ||
        $hymo === 'MS' ||
        ($hyva === 'S' && $precoVendaVariador == 0) ||
        ($hypl === 'S' && $precoVendaAdaptacao == 0) ||
        ($hymo !== 'N' && $categoria === 'PR')) {
        $precoTotal = 0;
    } else {
        $precoTotal = ($hymo === 'S' ? ($precoVendaRedutor * 1.1111) : $precoVendaRedutor) +
