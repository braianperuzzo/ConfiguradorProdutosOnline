...
function calcular_preco(PDO $pdo, array $post, string $companyCode, string $categoria, string $codigoEstado, string $pessoa): float
{
    ...
    $tabelaPrecoMotor = "_USR_CONF_MOPV_{$estado}";
    $tabelaPrecoOpcionalMotor = "_USR_CONF_MOOPCPV_{$estado}";

    $colunaBase = "MO{$categoria}" . ($post['MOCP'] ?? '');

    $colunaOpcional = null;
    if (strpos($moopc, 'VF') !== false) {
        $colunaOpcional = "MO{$categoria}VF";
    } elseif (strpos($moopc, 'CR') !== false) {
        $colunaOpcional = "MO{$categoria}CR";
    }

    $colunaPintura = null;
    if (strpos($moopc, 'PINTCZ') !== false) {
        $colunaPintura = "MO{$categoria}PINTCZ";
    } elseif (strpos($moopc, 'ACOR') !== false) {
        $colunaPintura = "MO{$categoria}ACOR";
    }

    $lookup = ['MOLN','MOTP','MOTT','MOFQ','MOPT','MOPL','MOCM'];
    $valores = [
        $post['MOLN'] ?? '',
        $post['MOTP'] ?? '',
        $post['MOTT'] ?? '',
    ...
    $precoMotor = procvm($pdo, $tabelaPrecoMotor, $lookup, $colunaBase, $valores);

    if ($colunaOpcional) {
        if (strpos($moopc, 'VF') !== false) {
            $precoOpcional = procvm($pdo, $tabelaPrecoMotor, $lookup, $colunaOpcional, $valores);
        } elseif (strpos($moopc, 'CR') !== false) {
            $precoOpcional = procvm(
                $pdo,
                $tabelaPrecoOpcionalMotor,
                ['MOLN', 'MOCM', 'MOCP'],
                $colunaOpcional,
                [
                    $post['MOLN'] ?? '',
    ...
    $precoPintura = $colunaPintura
        ? procvm($pdo, $tabelaPrecoMotor, $lookup, $colunaPintura, $valores)
        : 0;

    if (!$precoMotor ||
        ((strpos($moopc, 'VF') !== false || strpos($moopc, 'CR') !== false) && !$precoOpcional) ||
        ((strpos($moopc, 'PINTCZ') !== false || strpos($moopc, 'ACOR') !== false) && !$precoPintura) ||
        $mormes === 'S' ||
        $moes === 'S' ||
        $moln === '2.WS' ||
        !$pessoa) {
    ...
    return $precoMotor + $precoOpcional + $precoPintura;
}
