...
function calcular_preco(PDO $pdo, array $post, string $categoria): float {
    $colunaBase = "IN{$categoria}BR";
    $colunaPciAplicacao = "IN{$categoria}AP";
    $colunaPciPotencia = "IN{$categoria}NPC";
    $colunaCarcaca = "IN{$categoria}CM";
    $colunaTampa = "IN{$categoria}CP";
    $colunaSuporte = "IN{$categoria}OPCS";

    $lookup = ['INLN','INTP','INTT','INPT','INFE','INPC','INAP','INCM','INCP','INCO'];
    $valores = [
        $post['INLN'] ?? '',
        $post['INTP'] ?? '',
        $post['INTT'] ?? '',
    ...
    $precoBase = procvm($pdo, '_USR_CONF_INPV', $lookup, $colunaBase, $valores);
    $precoPciAplicacao = procvm($pdo, '_USR_CONF_INPV', $lookup, $colunaPciAplicacao, $valores);
    $precoPciPotencia = procvm($pdo, '_USR_CONF_INPV', $lookup, $colunaPciPotencia, $valores);
    $precoCarcaca = procvm($pdo, '_USR_CONF_INPV', $lookup, $colunaCarcaca, $valores);
    $precoTampa = procvm($pdo, '_USR_CONF_INPV', $lookup, $colunaTampa, $valores);

    $lookupSuporte = ['INLN','INTP','INTT','INFE','INCO','INOPCS','MOLN','MOCM'];
    $valoresSuporte = [
        $post['INLN'] ?? '',
        $post['INTP'] ?? '',
        $post['INTT'] ?? '',
    ...
    $precoSuporte = procvm($pdo, '_USR_CONF_INPVOPCS', $lookupSuporte, $colunaSuporte, $valoresSuporte);

    $total = (float)$precoBase + (float)$precoPciAplicacao + (float)$precoPciPotencia +
             (float)$precoCarcaca + (float)$precoTampa + (float)$precoSuporte;
    return round($total, 2);
}

$postData = $_POST;
$preco = calcular_preco($pdo, $postData, $categoria);
