...
function aplicar_descontos(PDO $pdo, string $produto, string $pessoa, float $preco): float {
    ...
    try {
        // obtém família, grupo, NCM e marca do produto
        ...
        // obtém tabela de descontos e categoria da pessoa
        ...
        // regras adicionais de acréscimo em DCTO_4
        $ncm = trim($rowProd['CD_NCM'] ?? '');
        $marca = strtoupper(trim($rowProd['CD_MARCA'] ?? ''));

        // ACRÉSCIMO 4% PARA REDUTOR REVENDAS
        // Se a categoria da pessoa for 3 ou 8
        // e o NCM do produto for 84834010
        // acresce 4% em DCTO_4
        if (in_array($categoria, ['3','8'], true) && $ncm === '84834010') {
            $novo += $novo * 0.04;
        }

        // ACRÉSCIMO 5% PARA REDUTOR CONSUMIDOR FINAL
        // Se a categoria da pessoa for 2 ou 5
        // e o NCM do produto for 84834010
        // acresce 5% em DCTO_4
        ...
        // ACRÉSCIMO 6% PARA MOTOR WEG CONSUMIDOR FINAL
        // Se a categoria da pessoa for 2 ou 5 ou 111
        // e a marca do produto for I4, I44 ou I61
        // acresce 6% em DCTO_4
...
function calcular_preco(PDO $pdo, array $rowPessoa, array $post, string $companyCode, string $categoria, string $codigoEstado)
{
    ...
    $colunaBase = "QU{$categoria}BR";
    $colunaEixoEntrada = "QU{$categoria}EE";
    $colunaAcessorioFixacao = "QU{$categoria}{$quaf}";
    $colunaAcessoriosSaida = "QU{$categoria}{$quas}";
    $colunaCapaProtecaoLateral = "QU{$categoria}{$qucl}";
    $colunaPintura = null;
    if (strpos($quopc, 'PINTCZ') !== false) {
        $colunaPintura = "QU{$categoria}PINTCZ";
    } elseif (strpos($quopc, 'ACOR') !== false) {
        $colunaPintura = "QU{$categoria}ACOR";
    }

    $lookupCampos = ['QULN', 'QUBR', 'QUEX'];
    $valoresLookup = [
        $post['QULN'] ?? '',
        $post['QUBR'] ?? '',
        $post['QUEX'] ?? ''
    ...
    $precoRedutor = procvm($pdo, '_USR_CONF_QUPV', $lookupCampos, $colunaBase, $valoresLookup);
    $precoEixoEntrada = strpos($qucp, 'EE') !== false
        ? procvm($pdo, '_USR_CONF_QUPV', $lookupCampos, $colunaEixoEntrada, $valoresLookup)
        : 0;
    $precoAcessorioFixacao = $quaf !== 'N'
        ? procvm($pdo, '_USR_CONF_QUPV', $lookupCampos, $colunaAcessorioFixacao, $valoresLookup)
        : 0;
    $precoAcessoriosSaida = $quas !== 'N'
        ? procvm($pdo, '_USR_CONF_QUPV', $lookupCampos, $colunaAcessoriosSaida, $valoresLookup)
        : 0;
    $precoCapaProtecaoLateral = $qucl !== 'N'
        ? procvm($pdo, '_USR_CONF_QUPV', $lookupCampos, $colunaCapaProtecaoLateral, $valoresLookup)
        : 0;
    $precoPintura = $colunaPintura
        ? procvm($pdo, '_USR_CONF_QUPV', $lookupCampos, $colunaPintura, $valoresLookup)
        : 0;

    $carcaca100112 = $post['QUCM'] ?? '';
    if ($qumo === 'S' && $carcaca100112 === '100-112') {
    ...
    if ((strpos($qucp, 'EE') !== false && !$precoEixoEntrada) ||
        ($quaf !== 'N' && !$precoAcessorioFixacao) ||
        ($quas !== 'N' && !$precoAcessoriosSaida) ||
        ($qucl !== 'N' && !$precoCapaProtecaoLateral) ||
        (((strpos($quopc, 'PINTCZ') !== false) || (strpos($quopc, 'ACOR') !== false)) && !$precoPintura)) {
        $precoVendaRedutor = 0;
    } else {
        $precoVendaRedutor = $precoRedutor + $precoEixoEntrada + $precoAcessorioFixacao +
            $precoAcessoriosSaida + $precoCapaProtecaoLateral + $precoPintura;
    }

    $tabelaPrecoMotor = $qumo === 'S' ? "_USR_CONF_MOPV_{$estado}" : '';
    $tabelaOpcMotor = $qumo === 'S' ? "_USR_CONF_MOOPCPV_{$estado}" : '';

    $colunaMotor = $qumo === 'S' ? "MO{$categoria}" . ($post['MOCP'] ?? '') : '';
    $colunaOpcMotor = null;
    if ($qumo === 'S') {
    ...
    $precoPinturaMotor = ($qumo === 'S' && $colunaPinturaMotor)
        ? procvm(
            $pdo,
            $tabelaPrecoMotor,
    ...
    $precoVendaVariador = ($quva === 'S')
        ? procvm(
            $pdo,
            '_USR_CONF_VAPV',
            ['VALN', 'VAPT', 'VACM', 'VASCP'],
            "VA{$categoria}",
            [$post['VALN'] ?? '', $post['VAPT'] ?? '', $post['QUCM'] ?? '', $post['VASCP'] ?? '']
        )
        : 0;

    $colunaAdaptacao = $qupl === 'S' ? "PL{$categoria}" : '';
    $diametro = $qupl === 'S'
        ? (($post['PLBU'] ?? 'N') === 'N' ? ($post['PLDE'] ?? '') : ($post['PLBU'] ?? ''))
        : '';
    $precoVendaAdaptacao = ($qupl === 'S')
        ? procvm($pdo, '_USR_CONF_PLASPV', ['PLCM', 'PLDE'], $colunaAdaptacao, [$post['QUCM'] ?? '', $diametro])
        : 0;

    $colunaHidraulico = $qumh === 'S' ? "MH{$categoria}" : '';
    $precoVendaHidraulico = ($qumh === 'S')
        ? procvm(
            $pdo,
            '_USR_CONF_MHPV',
            ['MHLN', 'MHBR', 'MHCM', 'MHDE'],
            $colunaHidraulico,
            [$post['QULN'] ?? '', $post['QUBR'] ?? '', $post['QUCM'] ?? '', $post['MHDE'] ?? '']
        )
        : 0;

    if ($precoVendaRedutor == 0 ||
        ($qumo === 'S' && $precoVendaMotor == 0) ||
        $qumo === 'MS' ||
        ($quva === 'S' && $precoVendaVariador == 0) ||
        ($qupl === 'S' && $precoVendaAdaptacao == 0) ||
        ($qumh === 'S' && $precoVendaHidraulico == 0) ||
        ($qumo !== 'N' && $categoria === 'PR')) {
        $precoTotal = 0;
    } else {
        $precoTotal = ($qumo === 'S' ? ($precoVendaRedutor * 1.1111) : $precoVendaRedutor) +
            $precoVendaMotor + $precoVendaVariador + $precoVendaAdaptacao + $precoVendaHidraulico;
