...
function calcular_preco(PDO $pdo, array $rowPessoa, array $post, string $companyCode, string $categoria, string $codigoEstado)
{
    ...
    $precoRedutor = procvm($pdo, '_USR_CONF_QUDRPV', $lookupCampos, $colunaBase, $valoresLookup);
    $precoEixoEntrada = strpos($qucp, 'EE') !== false
        ? procvm($pdo, '_USR_CONF_QUDRPV', $lookupCampos, $colunaEixoEntrada, $valoresLookup)
        : 0;
    $precoAcessorioFixacao = $quaf !== 'N'
        ? procvm($pdo, '_USR_CONF_QUDRPV', $lookupCampos, $colunaAcessorioFixacao, $valoresLookup)
        : 0;
    $precoAcessoriosSaida = $quas !== 'N'
        ? procvm($pdo, '_USR_CONF_QUDRPV', $lookupCampos, $colunaAcessoriosSaida, $valoresLookup)
        : 0;
    $precoCapaProtecaoLateral = $qucl !== 'N'
        ? procvm($pdo, '_USR_CONF_QUDRPV', $lookupCampos, $colunaCapaProtecaoLateral, $valoresLookup)
        : 0;
    $precoUniao = ($post['QULN'] ?? '') === '1.QP' && $quun !== '0'
        ? procvm($pdo, '_USR_CONF_QUDRPV', $lookupCampos, $colunaUniao, $valoresLookup)
        : 0;
    $precoPintura = $colunaPintura
        ? procvm($pdo, '_USR_CONF_QUDRPV', $lookupCampos, $colunaPintura, $valoresLookup)
        : 0;
    ...
    if ((strpos($qucp, 'EE') !== false && !$precoEixoEntrada) ||
        ...
        (($post['QULN'] ?? '') === '1.QP' && $quun !== '0' && !$precoUniao) ||
        ...
        $precoVendaRedutor = $precoRedutor + $precoEixoEntrada + $precoAcessorioFixacao +
            $precoAcessoriosSaida + $precoCapaProtecaoLateral + $precoUniao + $precoPintura;
    }
