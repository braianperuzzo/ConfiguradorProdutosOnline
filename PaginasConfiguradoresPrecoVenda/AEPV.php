...
function calcular_preco(PDO $pdo, array $post, string $companyCode, string $categoria, string $codigoEstado)
{
    $aeln  = $post['AELN']  ?? '';
    $aebr  = $post['AEBR']  ?? '';
    $aetp  = $post['AETP']  ?? '';
    ...
    $tabelaCubo = '_USR_CONF_AEPV_' . $estado;
    $tabelaElast = '_USR_CONF_AECELPV_' . $estado;
    $colunaCubo = 'AE' . $categoria . 'CB';
    $colunaElast = 'AE' . $categoria . 'EL';

    $precoCubo = procvm($pdo, $tabelaCubo, ['AELN','AEBR','AETP','AEEE','AEEE2'], $colunaCubo, [$aeln,$aebr,$aetp,$aeee,$aeee2]);
    $precoElast = procvm($pdo, $tabelaElast, ['AELN','AEBR','AEEL'], $colunaElast, [$aeln,$aebr,$aeel]);

    if ($precoCubo <= 0 || ($aeln !== '3.RIC' && $precoElast <= 0)) {
        return 0;
    }
    return $precoCubo + $precoElast;
}

$postData = $_POST;
$precoRS = calcular_preco($pdo, $postData, '001', $categoria, $codigoEstado);
$precoSP = calcular_preco($pdo, $postData, '003', $categoria, $codigoEstado);
