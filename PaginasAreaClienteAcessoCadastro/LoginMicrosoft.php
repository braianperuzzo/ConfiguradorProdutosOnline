...
if ($sessionPath === '' || !is_dir($sessionPath) || !is_writable($sessionPath)) {
    $defaultSessionDir = rtrim(sys_get_temp_dir(), DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR . 'php_sessions';
    if (!is_dir($defaultSessionDir)) {
        mkdir($defaultSessionDir, 0700, true);
    }
    if (is_dir($defaultSessionDir) && is_writable($defaultSessionDir)) {
        session_save_path($defaultSessionDir);
    }
}
...
function carregar_configuracao_microsoft(string $baseDir): array
{
    $config = [
        'client_id' => getenv('AZUREAD_CLIENT_ID') ?: null,
        'client_secret' => getenv('AZUREAD_CLIENT_SECRET') ?: null,
        'tenant_id' => getenv('AZUREAD_TENANT_ID') ?: null,
        'redirect_uri' => getenv('AZUREAD_REDIRECT_URI') ?: null,
    ];

    $iniPath = $baseDir . '/Configuracoes/MicrosoftEntra.ini';
    if (file_exists($iniPath)) {
        ...
        if (is_array($ini)) {
            ...
            $config['tenant_id'] = $config['tenant_id'] ?: ($ini['tenant_id'] ?? null);
            $config['redirect_uri'] = $config['redirect_uri'] ?: ($ini['redirect_uri'] ?? null);
        }
    }

        $config['redirect_uri'] = $config['redirect_uri']
        ?: 'https://configurador.redutoresibr.com.br/PaginasAreaClienteAcessoCadastro/LoginMicrosoft.php';

    $tenant = $config['tenant_id'] ?: 'common';
    $tenant = strtolower(trim((string) $tenant));

    $tenantAllowList = ['common', 'organizations', 'consumers'];
    $isGuidTenant = (bool) preg_match('/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i', $tenant);
    $isDomainTenant = (bool) preg_match('/^[a-z0-9][a-z0-9.-]+\.[a-z]{2,}$/i', $tenant);

    if (!in_array($tenant, $tenantAllowList, true) && !$isGuidTenant && !$isDomainTenant) {
        $tenant = 'common';
    }
    ...
    if (empty($config['client_id']) || empty($config['client_secret'])) {
        throw new RuntimeException('Configuração do Microsoft Entra incompleta.');
    }

    return $config;
}

function resolver_tenant_fluxo(array $config): string
{
    $tenantPermitidos = ['common', 'organizations', 'consumers'];

    $tenantParam = strtolower(trim((string) filter_input(INPUT_GET, 'tenant', FILTER_UNSAFE_RAW)));
    if (in_array($tenantParam, $tenantPermitidos, true)) {
        return $tenantParam;
    }

    $tenantConfig = strtolower(trim((string) ($config['tenant_id'] ?? 'common')));
    if (in_array($tenantConfig, $tenantPermitidos, true)) {
        return $tenantConfig;
    }
...
function responder_html(string $mensagem, int $status = 400): void
{
    ...
    echo '<!DOCTYPE html><html lang="pt-BR"><head><meta charset="utf-8"><title>Autenticação Microsoft | Configurador de Produtos IBR</title>';
    ...
    echo '<div class="title">Autenticação com Microsoft</div>';
...
function lidar_callback_duplicado(string $provedor, ?string $retornoSessao): void
{
    ...
    $agora = time();
    $sucessoRecente = $_SESSION['msft_oauth_success'] ?? null;
    ...
    if ($provedor === 'microsoft') {
        unset($_SESSION['msft_oauth_state'], $_SESSION['msft_oauth_last_code'], $_SESSION['msft_oauth_retorno'], $_SESSION['msft_oauth_tenant']);
    }
...
function iniciar_fluxo(array $config): void
{
    ...
    $_SESSION['msft_oauth_state'] = $state;
    global $cookieDomain, $cookieSecure, $cookieSameSite;
    $cookieState = [
        'expires' => time() + 900,
        'path' => '/',
    ...
    set_cookie_multi_domain('msft_oauth_state', $state, $cookieState, $cookieDomain);

    $retorno = validar_retorno(filter_input(INPUT_GET, 'retorno', FILTER_UNSAFE_RAW));
    if ($retorno !== null) {
        $_SESSION['msft_oauth_retorno'] = $retorno;
    }

    $tenantFluxo = resolver_tenant_fluxo($config);
    $_SESSION['msft_oauth_tenant'] = $tenantFluxo;
...
function erro_codigo_oauth_duplicado(string $mensagem): bool
{
    ...
    if (strpos($mensagem, 'aadsts54005') !== false) {
        return true;
    }
...
function requisitar_token(array $config, string $code, string $tenantFluxo): array
{
    $tenantFluxo = strtolower(trim($tenantFluxo));
    if (!in_array($tenantFluxo, ['common', 'organizations', 'consumers'], true)) {
        $tenantFluxo = 'common';
    }

    $url = sprintf('https://login.microsoftonline.com/%s/oauth2/v2.0/token', rawurlencode($tenantFluxo));
    ...
    if (!is_array($data)) {
        throw new RuntimeException('Resposta inválida do Microsoft Entra.');
    }

    if (isset($data['error'])) {
        throw new RuntimeException('Erro do Microsoft Entra: ' . ($data['error_description'] ?? $data['error']));
    }
...
function buscar_perfil(string $accessToken): array
{
    ...
    if ($response === false) {
        throw new RuntimeException('Erro ao consultar o Microsoft Graph: ' . $error);
    }

    $data = json_decode($response, true);
    if (!is_array($data)) {
        throw new RuntimeException('Perfil do Microsoft Graph inválido.');
    }
...
{
    $possiveis = [];
    if (!empty($perfil['mail'])) {
        $possiveis[] = $perfil['mail'];
    }
    if (!empty($perfil['userPrincipalName'])) {
        $possiveis[] = $perfil['userPrincipalName'];
    }
    if (!empty($dadosToken['id_token'])) {
        ...
        if (count($partes) >= 2) {
            ...
            if (!empty($payload['preferred_username'])) {
                $possiveis[] = $payload['preferred_username'];
            }
...
{
    $possiveis = [];
    if (!empty($perfil['displayName'])) {
        $possiveis[] = $perfil['displayName'];
    }

    $nomeComSobrenome = trim(($perfil['givenName'] ?? '') . ' ' . ($perfil['surname'] ?? ''));
    if ($nomeComSobrenome !== '') {
...
function autenticar_usuario_por_email(string $email, string $baseDir): bool
{
    ...
    try {
        ...
        log_event('LoginMicrosoft - falha ao disparar sincronizacao do cliente: ' . $t->getMessage());
    }
...
try {
    $config = carregar_configuracao_microsoft($baseDir);
    ...
    $stateSessao = $_SESSION['msft_oauth_state'] ?? '';
    $stateCookie = $_COOKIE['msft_oauth_state'] ?? '';
    ...
    if ($stateCookie !== '') {
        ...
        set_cookie_multi_domain('msft_oauth_state', '', $cookieState, $cookieDomain);
    }

    $_SESSION['msft_oauth_processing'] = time();

    $cacheDir = obter_diretorio_cache_oauth($baseDir);
    if (codigo_oauth_foi_utilizado($code, $cacheDir)) {
        registrar_callback_duplicado('microsoft', 'codigo_oauth_foi_utilizado');
        $retornoSessao = validar_retorno($_SESSION['msft_oauth_retorno'] ?? null);
        lidar_callback_duplicado('microsoft', $retornoSessao);
    }

    if (!empty($_SESSION['msft_oauth_last_code']) && hash_equals($_SESSION['msft_oauth_last_code'], $code)) {
        registrar_callback_duplicado('microsoft', 'msft_oauth_last_code');
        $retornoSessao = validar_retorno($_SESSION['msft_oauth_retorno'] ?? null);
        lidar_callback_duplicado('microsoft', $retornoSessao);
    }
    $_SESSION['msft_oauth_last_code'] = $code;

    $tenantFluxo = strtolower(trim((string) ($_SESSION['msft_oauth_tenant'] ?? 'common')));
    $dadosToken = requisitar_token($config, $code, $tenantFluxo);
    if (empty($dadosToken['access_token'])) {
        responder_html('Token de acesso não recebido do Microsoft Entra.');
    }
    ...
    if (!$email) {
        responder_html('Não foi possível identificar o e-mail do usuário Microsoft.');
    }

    $autenticado = autenticar_usuario_por_email($email, $baseDir);

    if (!$autenticado) {
        unset($_SESSION['msft_oauth_processing']);
    ...
    $_SESSION['msft_oauth_success'] = time();
    unset($_SESSION['msft_oauth_tenant']);
    ...
    if (erro_codigo_oauth_duplicado($e->getMessage())) {
        registrar_callback_duplicado('microsoft', 'codigo_ja_resgatado');
        $retornoSessao = validar_retorno($_SESSION['msft_oauth_retorno'] ?? null);
        lidar_callback_duplicado('microsoft', $retornoSessao);
    }
    log_event('LoginMicrosoft: ' . $e->getMessage());
    responder_html('⚠️ Não foi possível completar o login com a Microsoft. ' . $e->getMessage(), 500);
} catch (Throwable $t) {
    log_event('LoginMicrosoft: ' . $t->getMessage());
    responder_html('⚠️ Erro inesperado ao autenticar com a Microsoft.', 500);
}
