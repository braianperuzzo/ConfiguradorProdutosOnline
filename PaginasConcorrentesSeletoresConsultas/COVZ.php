...
$sql = "SELECT DISTINCT COVZ
        FROM _USR_CONF_SITE_CONCORRENTE_DADOS
        WHERE COMA = ?
          AND COLN = ?
          AND COBR = ?";
$params = [$marcaconcorrente, $linhaconcorrente, $tamanhoconcorrente];

if ($reducaoconcorrente !== '') {
    $sql .= "
          AND CORD = ?";
    $params[] = $reducaoconcorrente;
}
...
while ($row = $query->fetch(PDO::FETCH_ASSOC)) {
    $valor = htmlspecialchars((string) ($row["COVZ"] ?? ''), ENT_QUOTES, 'UTF-8');
