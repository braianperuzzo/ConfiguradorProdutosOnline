...
try {
    ...
    $stmt = $pdo->prepare("DELETE FROM _USR_CONF_SITE_HISTORICO_PRODFAV WHERE DS_EMAIL = ? AND NR_CPFCNPJ = ? AND DS_REFERENCIA = ?");
    $stmt->execute([$email, $documento, $produto]);

    echo json_encode([
        'sucesso' => true,
        'mensagem' => '❤️ Favorito Removido com Sucesso!'
    ]);
    $pdo = null;
} catch (PDOException $e) {
    log_event('ExcluirFavorito: ' . $e->getMessage());
    http_response_code(500);
    echo json_encode(['erro' => '⚠️ Erro ao Excluir Favorito']);
}