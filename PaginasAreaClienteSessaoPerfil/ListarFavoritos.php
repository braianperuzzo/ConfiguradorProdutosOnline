...
try {
 ...
 $stmtTot = $pdo->prepare(
        "SELECT COUNT(*) FROM _USR_CONF_SITE_HISTORICO_PRODFAV WHERE DS_EMAIL = ? AND NR_CPFCNPJ = ?"
    );
    $stmtTot->execute([$email, $documento]);
    $total = (int)$stmtTot->fetchColumn();

    $stmt = $pdo->prepare(
        "SELECT DS_REFERENCIA AS produto, DS_LINK AS link, DT_DATA AS data, DS_COMENTARIO AS comentario
           FROM _USR_CONF_SITE_HISTORICO_PRODFAV
          WHERE DS_EMAIL = ? AND NR_CPFCNPJ = ?
          ORDER BY TRY_CONVERT(datetime, DT_DATA, 120) DESC, DT_DATA DESC
    ...
    foreach ($lista as &$item) {
        ...
        $item['data'] = date('d/m/Y H:i:s', $ts ?: time());
    ...
    log_event('ListarFavoritos: ' . $e->getMessage());
    http_response_code(500);
    echo json_encode(['erro' => '⚠️ Erro ao Carregar Favoritos']);
}
