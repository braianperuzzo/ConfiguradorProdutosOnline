...
try {
    ...
    log_event('Token da PipeRun indisponível em ListarCadastros: ' . $e->getMessage());
...
try {
   ...
   $stmtTot = $pdo->prepare(
        "SELECT COUNT(*) FROM _USR_CONF_SITE_HISTORICO_CADASTROS WHERE DS_EMAIL = ? AND NR_CPFCNPJ = ?"
    );
